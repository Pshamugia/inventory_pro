  ...
  <div class="row">
    ...
    <div class="col-lg-7 pos-left">
      <div class="d-flex gap-2 mb-3">
        <input id="searchInput" class="form-control form-control-lg" placeholder="პროდუქტის დასახელება ან შტრიხკოდი… (Enter to search)">
    ...
    <div class="col-lg-5">
      ...
      <div class="mb-2">
        <label class="small text-muted">Customer (optional)</label>
      ...
      <div class="alert alert-warning mb-2" id="openDrawerBox">
        <div class="d-flex align-items-center gap-2">
          <input id="openingCash" type="number" class="form-control form-control-sm"
            placeholder="Opening cash ₾">
          <button id="openDrawerBtn" class="btn btn-dark btn-sm">
            Open Day
          </button>
        </div>
      </div>
    ...
    <button id="closeDayBtn" class="btn btn-dark">
        Close Day (Z-Report)
    </button>
</div>
...
<template id="zReceiptTpl">
  <div style="font-family: ui-sans-serif,system-ui,Arial; width: 220px; font-size:12px;">
    <div style="text-align:center;">
      <div style="font-weight:700">Z-REPORT</div>
      <div data-date></div>
      <div style="border-top:1px dashed #000;margin:6px 0"></div>
    </div>

    <div style="display:flex;justify-content:space-between"><span>Opening</span><span data-opening></span></div>
    <div style="display:flex;justify-content:space-between"><span>Cash sales</span><span data-cashsales></span></div>
    <div style="display:flex;justify-content:space-between"><span>Card sales</span><span data-cardsales></span></div>
    <div style="display:flex;justify-content:space-between"><span>Cash received</span><span data-cashin></span></div>
    <div style="display:flex;justify-content:space-between"><span>Change given</span><span data-change></span></div>

    <div style="border-top:1px dashed #000;margin:6px 0"></div>

    <div style="display:flex;justify-content:space-between"><strong>Expected</strong><strong data-expected></strong></div>
    <div style="display:flex;justify-content:space-between"><span>Actual</span><span data-actual></span></div>
    <div style="display:flex;justify-content:space-between"><span>Diff</span><span data-diff></span></div>

    <div style="border-top:1px dashed #000;margin:6px 0"></div>
    <div style="text-align:center;font-size:11px">Inventory-Pro</div>
  </div>
</template>
...
<div class="modal fade" id="zReportModal" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-header">
        <strong>Z-Report Preview</strong>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>

     <div class="modal-body small">
  <div id="zReportBody">Loading…</div>

  <hr>

  <div class="mb-2">
    <label class="small text-muted">Actual cash in drawer (counted)</label>
    <input id="actualCashInput" type="number" step="0.01" class="form-control form-control-sm" placeholder="₾ 0.00">
  </div>

  <div class="mb-2">
    <label class="small text-muted">Note (optional)</label>
    <input id="closeNoteInput" type="text" class="form-control form-control-sm" placeholder="e.g. missing 5 GEL">
  </div>

  <div class="small">
    <strong>Expected cash:</strong> <span id="expectedCashText">₾ 0.00</span><br>
    <strong>Difference:</strong> <span id="cashDiffText">₾ 0.00</span>
  </div>
</div>
      ...
      <div class="modal-footer">
        ...
        <button id="printZBtn" class="btn btn-primary">
          Print Z-Report
        </button>
        <button id="confirmCloseDayBtn" class="btn btn-danger">
          Close Day
        </button>
      </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (openDrawerBtn) {
      openDrawerBtn.onclick = async () => {
        const val = Number(openingCashEl.value || 0);

        if (val < 0) return alert('Invalid opening cash');
        ...
        if (json.ok) {
          document.getElementById('openDrawerBox').remove();
        }
      };
    }
    ...
    async function loadCustomers(q = '') {
      const res = await fetch("{{ route('pos.customers') }}?q=" + encodeURIComponent(q));
    ...
    document.querySelector('.keypad').addEventListener('click', (e) => {
      ...
      // === CASH MODE (no cart row selected) ===
      if (selectedIndex < 0) {
        let cur = String(cashGivenEl.value || '0');
    ...
    function printZ(d, actualCash) {
  const tpl = document.getElementById('zReceiptTpl').content.cloneNode(true);

  const set = (sel, val) => {
    const el = tpl.querySelector(sel);
    if (el) el.textContent = val;
  };

  set('[data-date]', d.date);
  set('[data-opening]', '₾ ' + Number(d.opening_cash).toFixed(2));
  set('[data-cashsales]', '₾ ' + Number(d.cash_sales).toFixed(2));
  set('[data-cardsales]', '₾ ' + Number(d.card_sales).toFixed(2));
  set('[data-cashin]', '₾ ' + Number(d.cash_received).toFixed(2));
  set('[data-change]', '₾ ' + Number(d.change_given).toFixed(2));

  set('[data-expected]', '₾ ' + Number(d.expected_cash).toFixed(2));
  set('[data-actual]', '₾ ' + Number(actualCash).toFixed(2));
  set('[data-diff]', '₾ ' + (actualCash - Number(d.expected_cash)).toFixed(2));

  const w = window.open('', '_blank', 'width=260,height=700');
  w.document.write('<!doctype html><html><head><meta charset="utf-8"><title>Z-Report</title></head><body>');
...
const expectedCashText  = document.getElementById('expectedCashText');
const cashDiffText      = document.getElementById('cashDiffText');
...
const zReportModal = new bootstrap.Modal(
  document.getElementById('zReportModal')
);
const zReportBody = document.getElementById('zReportBody');

if (closeDayBtn) {
  closeDayBtn.onclick = async () => {
    const res = await fetch("{{ route('reports.z') }}");
    const d = await res.json();

zReportBody.innerHTML = `
  <div><strong>Date:</strong> ${d.date}</div>
  <hr>
  <div><strong>Opening cash:</strong> ₾ ${Number(d.opening_cash).toFixed(2)}</div>
  <div><strong>Cash sales:</strong> ₾ ${Number(d.cash_sales).toFixed(2)}</div>
  <div><strong>Card sales:</strong> ₾ ${Number(d.card_sales).toFixed(2)}</div>
  <div><strong>Cash received:</strong> ₾ ${Number(d.cash_received).toFixed(2)}</div>
  <div><strong>Change given:</strong> ₾ ${Number(d.change_given).toFixed(2)}</div>
  <hr>
  <div><strong>Net cash:</strong> ₾ ${Number(d.net_cash).toFixed(2)}</div>
  <div><strong>Transactions:</strong> ${d.count}</div>
`;

// ⬇️ ADD THIS EXACT BLOCK RIGHT HERE

// store Z data for printing later
lastZData = d;

// expected cash from backend
const expected = Number(d.expected_cash || 0);
expectedCashText.textContent = '₾ ' + expected.toFixed(2);

// preset actual cash if empty
if (!actualCashInput.value) {
  actualCashInput.value = Number(d.actual_cash || 0).toFixed(2);
}

// calculate difference
const actual = Number(actualCashInput.value || 0);
cashDiffText.textContent = '₾ ' + (actual - expected).toFixed(2);

// live update when cashier types
actualCashInput.oninput = () => {
  const a = Number(actualCashInput.value || 0);
  cashDiffText.textContent = '₾ ' + (a - expected).toFixed(2);
};
...
printZBtn.onclick = () => {
  if (!lastZData) {
    alert('Load Z-report first');
    return;
  }

  const actual = Number(actualCashInput.value || 0);
  printZ(lastZData, actual);
};


if (confirmCloseDayBtn) {
  confirmCloseDayBtn.onclick = async () => {
    const actual = Number(actualCashInput.value || 0);
    const note = (closeNoteInput.value || '').trim();

    if (!confirm('Close the day now? This cannot be undone.')) {
      return;
    }

    const res = await fetch("{{ route('pos.closeDay') }}", {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-CSRF-TOKEN': csrf,
    ...
    if (!json.ok) {
      alert(json.error || 'Failed to close day');
      return;
    }

    alert(
      'Day closed.\n' +
      'Expected: ₾ ' + Number(json.expected_cash || 0).toFixed(2) + '\n' +
      'Actual: ₾ ' + Number(json.actual_cash || 0).toFixed(2) + '\n' +
      'Diff: ₾ ' + Number(json.cash_diff || 0).toFixed(2)
    );
...
/* ✅ ADD THIS BLOCK RIGHT HERE — AFTER the block above */
